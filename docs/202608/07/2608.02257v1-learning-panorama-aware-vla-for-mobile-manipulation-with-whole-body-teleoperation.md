...
tldr: "移动操作要求VLA同时处理数据采集与全局空间理解。针对全身遥操作困难与局部视野局限，本文开发了VR全身遥操作方案，并构建PanoVLA全景感知VLA策略，通过全景编码与融合增强空间上下文。在四个真实移动操作任务中，阶段完成率达91.3%，端到端成功率73.4%。结果表明，全景感知显著提升了移动机器人的空间理解与闭环操作性能。"
source: arxiv
...
motivation: 移动操作VLA面临数据采集困难和局部视野限制，缺乏全局空间理解。
method: 开发全身遥操作系统采集5.5小时数据，提出PanoVLA，基于MoT架构融合全景观测与语言、状态。
result: "在四个真实任务中，PanoVLA阶段完成率91.3%，端到端成功率73.4%，优于局部基线。"
conclusion: 全景空间上下文可显著提升移动机器人的空间理解与闭环操作性能。
---

## 摘要
移动操作是具身智能的关键能力，使机器人能够在开放世界环境中完成复杂的多阶段任务。然而，移动操作对视觉-语言-动作（VLA）策略提出了两个关键挑战：在数据层面，高质量全身演示的高效采集需要移动基座和机械臂的协调控制；在模型层面，现有VLA模型主要依赖局部相机观测，其有限的视野阻碍了全局空间理解。为解决这些挑战，我们开发了一套全身遥操作系统和一种全景感知的VLA策略。该系统通过单一VR接口实现对轮式双臂机器人的协调控制，并支持获取包含5.5小时多模态演示的真实世界移动操作数据集。在此数据集基础上，我们提出了PanoVLA，一种用于移动双臂操作的全景感知视觉-语言-动作策略。PanoVLA基于Mixture-of-Transformers架构，通过专门的全景编码和融合模块引入全局空间上下文，实现全景观测与语言指令和机器人状态的有效集成以生成动作。在四个真实世界移动操作任务上的评估表明，PanoVLA实现了91.3%的平均阶段完成率和73.4%的端到端成功率，大幅优于局部视图基线。这些结果表明，融入全景空间上下文能够提升移动机器人的空间理解和闭环操作性能。
