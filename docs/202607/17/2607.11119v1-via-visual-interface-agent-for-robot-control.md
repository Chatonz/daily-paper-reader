---
title: "VIA: Visual Interface Agent for Robot Control"
title_zh: VIA：用于机器人控制的视觉界面智能体
authors: "Hengyuan Hu, Priya Sundaresan, Jensen Gao, Dorsa Sadigh"
date: 2026-07-13
pdf: "https://arxiv.org/pdf/2607.11119v1"
tags: ["query:vla-humanoid"]
score: 7.0
evidence: 利用基础模型通过视觉界面控制机器人
tldr: "通用基础模型虽具备视觉和推理能力，但直接用于机器人控制需微调成视觉-语言-动作模型，受限于数据和计算。VIA提出将机器人控制重构为代理任务：现成FM驱动的代理通过浏览器3D界面截图、发布指令、观察结果来控制机械臂，无需专用微调。零样本下在LIBERO-Goal任务上达96.7%成功率，长程任务100%，且性能随基础模型增强而提升。这项工作揭示了前沿代理已内隐具备机器人控制技能，为机器人学习提供了新思路。"
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2607.11119v1/fig-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1421, \"height\": 438, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2607.11119v1/fig-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1376, \"height\": 804, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2607.11119v1/fig-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 1430, \"height\": 403, \"label\": \"Figure\"}]"
tables_json: "[{\"url\": \"assets/tables/arxiv/2607.11119v1/table-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1360, \"height\": 430, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.11119v1/table-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1405, \"height\": 456, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.11119v1/table-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 1409, \"height\": 459, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.11119v1/table-004.webp\", \"caption\": \"\", \"page\": 0, \"index\": 4, \"width\": 1406, \"height\": 456, \"label\": \"Table\"}]"
motivation: 传统方法微调FM为VLA模型成本高且规模受限，亟需能直接利用FM通用能力的机器人控制框架。
method: 通过浏览器3D界面，FM代理以截图和简单指令操作机械臂，实现闭环控制，无需专用训练或特权状态。
result: "零样本在LIBERO-Goal任务达96.7%，长程彩虹组装100%，性能随模型规模提升。"
conclusion: 前沿代理的技能可直接迁移至机器人控制，提供低成本、强泛化性的新范式。
---

## 摘要
机器人操作是一项复杂的任务，需要视觉理解、物理推理、规划和闭环控制。通用基础模型（FM）在这些能力中的某些方面（尤其是视觉和推理）已变得非常强大。为了利用这些能力构建通用机器人策略，当前的方法通常涉及将现有的FM微调为视觉-语言-动作（VLA）模型，通过在机器人数据上进行微调以输出低级动作。然而，由于微调可用的数据和计算资源有限，VLA通常比前沿FM小数个数量级，这反过来限制了它们的通用能力。受FM通过视觉界面操作软件的能力日益增强的启发，我们探究了同样的能力是否足以控制机器人。我们提出了VIA（Visual Interface Agent for robot control，机器人控制视觉界面智能体），这是一个将机器人控制重新定义为智能体任务的框架：一个基于现成FM的智能体通过基于浏览器的3D界面驱动机械臂，它通过截取屏幕截图、发出直观命令、观察结果并进行调整来操作。该智能体没有接受任何机器人特定的微调，也无法访问特权状态信息：它感知视觉输入并通过一小套通用工具进行动作。VIA继承了智能体的通用推理、闭环错误恢复以及根据观察进行规划和重新规划的能力。它能够零样本解决一系列桌面操作任务，同时适用于Claude Code和Codex。使用最强模型（Fable 5）时，它在三个LIBERO-Goal任务上实现了96.7%的成功率，并在一个长时程的彩虹组装任务上实现了100%的成功率。性能随底层模型的规模和强度而提升。这些结果表明，前沿智能体已经具备了在适当接口下直接迁移到机器人控制的技能：在某种意义上，你的编码或计算机使用智能体正秘密地也是一个机器人控制智能体。

## Abstract
Robot manipulation is a complex task that requires visual understanding, physical reasoning, planning, and closed-loop control. General-purpose foundation models (FMs) have grown remarkably capable of some of these, especially vision and reasoning. To leverage this for generalist robot policies, current methods typically involve converting existing FMs into vision-language-action (VLA) models by fine-tuning on robot data to output low-level actions. However, VLAs are often orders of magnitude smaller than frontier FMs given the limited data and compute available for fine-tuning, which in turn limits their general capability. Inspired by the growing ability of FMs to operate software through visual interfaces, we ask whether that same competence suffices to control a robot. We present VIA (Visual Interface Agent for robot control), a framework that recasts robot control as an agentic task: an off-the-shelf FM-powered agent drives a manipulator through a browser-based 3D interface by taking screenshots, issuing intuitive commands, observing the outcome, and adjusting. The agent receives no robot-specific fine-tuning and no access to privileged state information: it perceives visual input and acts through a small set of general tools. VIA inherits the agent's general reasoning, closed-loop error recovery, and ability to plan and re-plan from what it observes. It solves a diverse suite of tabletop manipulation tasks zero-shot with both Claude Code and Codex. With the strongest model (Fable 5) it achieves 96.7% success on three LIBERO-Goal tasks and 100% on a long-horizon rainbow assembly task. Performance improves with the scale and strength of the underlying model. These results suggest that frontier agents already possess skills that transfer directly to robot control given the right interface: your coding or computer-use agent is, in a sense, secretly a robot-control agent.